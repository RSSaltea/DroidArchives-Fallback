...
function normaliseNotificationPreferences(value){
  ...
  return {priority:['next','rare','furthest'].includes(value?.priority)?value.priority:'next',
    horizon:Math.max(1,Math.min(35,Math.floor(Number(value?.horizon)||3))),ownedPolicy:value?.ownedPolicy==='upgrades'?'upgrades':'missing',tracked};
}
...
function notificationRecommendations(){
  ...
  const candidates=[...byName.values()].map(entry=>{
    ...
    return {...entry,variant:target.variant,at:target.at,nextAt:entry.needs[0].at,lastAt:entry.needs.at(-1).at};
  });
  candidates.sort((a,b)=>settings.priority==='rare'
    ?rarityStep(b.rarity)-rarityStep(a.rarity)||b.lastAt-a.lastAt||variantStep(b.variant)-variantStep(a.variant)||a.name.localeCompare(b.name)
    :settings.priority==='furthest'?b.lastAt-a.lastAt||rarityStep(b.rarity)-rarityStep(a.rarity)||a.name.localeCompare(b.name)
    :a.nextAt-b.nextAt||rarityStep(b.rarity)-rarityStep(a.rarity)||variantStep(b.variant)-variantStep(a.variant)||a.name.localeCompare(b.name));
...
function notificationRecommendationsHtml(){
  ...
  const card=(rec,active)=>{
    const d=fusionDroid(rec.name),owned=state.owned.some(row=>row.name===rec.name),matched=model.eligible.find(row=>row.name===rec.name);
    const detail=active?`${owned?'Owned now. ':''}${!matched&&!owned?'Outside the current recommendations. ':''}Click to mark as no longer tracked.`:
      `Needed at R: ${rec.needs.map(n=>n.at).join(', ')} · ${variantLabel(rec.variant)}.${rec.have?` Owned at ${variantLabel(rec.have)}; you can upgrade it instead.`:''} Click after setting this notification in-game.`;
    return `<li><button class="notification-card ${active?'is-tracked':''}" type="button" data-notification-track="${escapeAttr(rec.name)}" data-notification-variant="${rec.variant||''}" aria-pressed="${active}" ${state.sharedView&&!state.sharedView.canEdit?'disabled':''}>${d?picture(d,rec.variant||'DEFAULT'):''}<span><strong>${escapeAttr(rec.name)}${variantWatch&&rec.variant?` · ${variantText(rec.variant)}`:''}</strong><small>${detail}</small></span><em>${active?'✓ Tracking':'+ Track'}</em></button></li>`;
  };
...
function showNotificationSettings(){
  ...
  root.querySelector('#notificationVariantWatch').onchange=e=>{root.querySelector('#notificationUpgrades').disabled=!e.target.checked;root.querySelector('#notificationManualVariant').disabled=!e.target.checked;};
  ...
  root.querySelector('#saveNotificationSettings').onclick=()=>{
    state.notificationPreferences=normaliseNotificationPreferences({...settings,priority:root.querySelector('#notificationPriority').value,horizon:root.querySelector('#notificationHorizon').value,ownedPolicy:root.querySelector('#notificationUpgrades').checked?'upgrades':'missing'});
...
function attachCollapsiblePanels(){
  ...
  if(actions)actions.innerHTML=`<button class="btn secondary base-panel-toggle ${baseViewIsMap()?'active':''}" id="toggleBaseMap">${baseViewIsMap()?'Hide Map':'Show Map'}</button><button class="btn secondary base-panel-toggle" id="toggleHealthPanel">Hide Health</button><button class="btn secondary base-panel-toggle" id="toggleScrapPanel">Hide Scrap</button><button class="btn secondary base-panel-toggle" id="toggleChipSellPanel">Hide Chips</button><button class="btn secondary base-panel-toggle" id="toggleReplacementPanel">Hide Droid Calc</button><button class="btn secondary base-panel-toggle" id="toggleOutlookPanel">Hide Outlook</button><button class="btn secondary base-panel-toggle" id="toggleNotificationsPanel">Hide Notifications</button>${hasGroupOutlook?'<button class="btn secondary base-panel-toggle" id="toggleGroupOutlookPanel">Hide Group Outlook</button>':''}${hasFusionOutlook?'<button class="btn secondary base-panel-toggle" id="toggleFusionOutlookPanel">Hide Fusion</button>':''}<button class="btn secondary base-panel-toggle" id="toggleBaseDetail">Hide Detail</button><button class="btn secondary" id="transferBase" title="Import or export Base">Import / Export</button>`;
  ...
  [{selector:'.notification-panel',button:'#toggleNotificationsPanel',key:'droid-archive-notifications-collapsed',label:'Notifications'},
   ...
   {selector:'.base-health',button:'#toggleHealthPanel',key:'droid-archive-health-collapsed',label:'Health'},{selector:'.scrap-calculator:not(.chip-sell-calculator)',button:'#toggleScrapPanel',key:'droid-archive-scrap-collapsed',label:'Scrap'},{selector:'.chip-sell-calculator',button:'#toggleChipSellPanel',key:'droid-archive-chip-sell-collapsed',label:'Chips'},{selector:'.replacement-calculator',button:'#toggleReplacementPanel',key:'droid-archive-replacement-collapsed',label:'Droid Calc',defaultCollapsed:true},{selector:'.rebirth-summary-box',button:'#toggleOutlookPanel',key:'droid-archive-rebirth-outlook-collapsed',label:'Outlook'},{selector:'.group-outlook-panel',button:'#toggleGroupOutlookPanel',key:`droid-archive-group-outlook-collapsed:${state.cloud.user?.id||'local'}`,label:'Group Outlook'}].forEach(item=>{const panel=document.querySelector(item.selector),button=document.querySelector(item.button);if(!panel||!button)return;const update=collapsed=>{panel.classList.toggle('collapsed',collapsed);button.classList.toggle('active',!collapsed);button.textContent=`${collapsed?'Show':'Hide'} ${item.label}`;button.title=`${collapsed?'Show':'Hide'} ${item.label}`;button.setAttribute('aria-expanded',String(!collapsed));if(item.label==='Notifications')button.innerHTML=commandIcon('notifications')+' '+(document.documentElement.dataset.uiStyle==='modern'?'Notifications':button.textContent);if(document.documentElement.dataset.uiStyle==='modern')requestAnimationFrame(()=>decorateCommandDeck('/base'))},stored=localStorage.getItem(item.key);update(stored===null?Boolean(item.defaultCollapsed):stored==='1');button.onclick=()=>{const collapsed=!panel.classList.contains('collapsed');localStorage.setItem(item.key,collapsed?'1':'0');update(collapsed)}});
