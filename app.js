...
function attachCollapsiblePanels(){
  ...
  [{selector:'.crafting-summary',button:'#toggleCraftingPanel',key:'droid-archive-crafting-collapsed',label:'Crafting'},
   ...
   {selector:'.base-health',button:'#toggleHealthPanel',key:'droid-archive-health-collapsed',label:'Health'},{selector:'.scrap-calculator:not(.chip-sell-calculator)',button:'#toggleScrapPanel',key:'droid-archive-scrap-collapsed',label:'Scrap'},{selector:'.chip-sell-calculator',button:'#toggleChipSellPanel',key:'droid-archive-chip-sell-collapsed',label:'Rebirth Prep'},{selector:'.replacement-calculator',button:'#toggleReplacementPanel',key:'droid-archive-replacement-collapsed',label:'Droid Calc',defaultCollapsed:true},{selector:'.rebirth-summary-box',button:'#toggleOutlookPanel',key:'droid-archive-rebirth-outlook-collapsed',label:'Outlook'},{selector:'.group-outlook-panel',button:'#toggleGroupOutlookPanel',key:`droid-archive-group-outlook-collapsed:${state.cloud.user?.id||'local'}`,label:'Group Outlook'}].forEach(item=>{const panel=document.querySelector(item.selector),button=document.querySelector(item.button);if(!panel||!button)return;const update=collapsed=>{panel.classList.toggle('collapsed',collapsed);if(item.label==='Crafting')panel.hidden=collapsed;button.classList.toggle('active',!collapsed);button.textContent=`${collapsed?'Show':'Hide'} ${item.label}`;button.title=`${collapsed?'Show':'Hide'} ${item.label}`;button.setAttribute('aria-expanded',String(!collapsed));if(item.label==='Notifications')button.innerHTML=commandIcon('notifications')+' '+(document.documentElement.dataset.uiStyle==='modern'?'Notifications':button.textContent);if(['modern','next'].includes(document.documentElement.dataset.uiStyle))requestAnimationFrame(()=>decorateCommandDeck('/base'))},stored=localStorage.getItem(item.key);update(stored===null?Boolean(item.defaultCollapsed):stored==='1');button.onclick=()=>{const collapsed=!panel.classList.contains('collapsed');localStorage.setItem(item.key,collapsed?'1':'0');update(collapsed)}});
