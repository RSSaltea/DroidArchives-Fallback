...
function optimisedPlacements(baseP,plan){
  ...
  for(const unit of units){
    ...
    if(keepForFusion(unit)){candidates.push({unit:{...unit,keepReason:'fusion'},fallbacks:loungeLikeStations(),old:current.get(key),betterStorageOpen:false,kept:false,spared:true,keepReason:'fusion'});continue;}
...
function optimisePage(){
  ...
  document.querySelector('.build-side').insertAdjacentHTML('afterend',`<div class="special-stations">${station('LOUNGE')}${station('COMPANION')}${station('UPGRADE_CHIP')}<div class="fusion-panel">${station('FUSION')}${station('FUSION_BUILD')}</div></div>`);
...
const DATA_VERSION='2026-09-13-protocol';
