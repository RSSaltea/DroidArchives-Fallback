import { kyberIsReleased, isKyberPreviewUser, visiblePatchNotes } from './release-gate.js?v=2026-09-26-release';
...
import { kyberGuide, kyberDroidDetails } from './kyber-guide.js?v=2026-09-26-release';
...
function droidAttribute(d,variant='DEFAULT'){
  ...
  const rarityLevel={COMMON:1,RARE:2,EPIC:3,LEGENDARY:4,MYTHIC:5}[d?.rarity],variantLevel=Math.max(0,variantRank(variant));
  ...
  if(d.type==='PROTOCOL')return`${200*(rarityLevel+variantLevel)}% Credit Multiplier (Companion)`;
  return'N/A'
}
...
function protocolSummaryHtml(placed){
  ...
  return `<section class="panel protocol-summary"><h2>Regional production</h2><div class="protocol-summary-grid">${PROTOCOL_REGIONS.map((region,slot)=>{
    ...
    return `<div><h3>${stationName(region)}</h3><strong>${protocolNumber(row.total)}/s</strong><small>${protocolNumber(row.beforeBonus)}/s × ${row.multiplier.toFixed(2)} · +${protocolNumber(row.bonus)}/s from Protocol</small><span>Build ${slot+1}: +${protocolNumber(bonus)}/s crafting</span>${seconds?`<small>${build.name}: ${craftTimeText(seconds/(1+bonus))} with Protocol (${craftTimeText(seconds)} base)</small>`:''}</div>`;
  }).join('')}</div><p class="protocol-note">Craft estimates apply the regional Protocol bonus; other crafting boosts are additional. Bonus slots are separate from normal credit work slots.</p></section>`;
}
...
function droidAttributeValue(d,variant){
  ...
  if(d.type==='PROTOCOL')return 200*(rarityLevel+variantLevel);
  return 0;
}
...
let kyberPreviewVerified=false,releaseData=null,releaseAudience='',allReleaseNotes=[],previewCheck=0;
...
const DATA_VERSION='2026-09-26-release';
