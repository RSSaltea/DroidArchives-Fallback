...
function normaliseNotificationPreferences(value){
  return {priority:['next','rare','furthest'].includes(value?.priority)?value.priority:'next',
    horizon:Math.max(1,Math.min(35,Math.floor(Number(value?.horizon)||3))),includeUpgrades:value?.includeUpgrades!==false};
}
...
function notificationRecommendations(){
  ...
  for(const req of futureRequirements()){
    ...
    const have=manual?rebirthTrackerStatus(req.at,req).selected:
      state.owned.filter(row=>row.name===d.name).reduce((best,row)=>!best||variantStep(row.variant)>variantStep(best)?row.variant:best,null);
    if(manual&&rebirthTrackerStatus(req.at,req).ready)continue;
    if(have&&variantStep(have)>=variantStep(req.variant))continue;
    if(have&&!settings.includeUpgrades)continue;
  ...
  return {settings,slots,variantWatch,candidates,recommendations:candidates.slice(0,slots)};
}
function notificationRecommendationsHtml(){
  const model=notificationRecommendations(),{settings,slots,variantWatch,recommendations}=model;
  ...
  return `<section class="notification-panel"><header><div><p class="eyebrow">Prepare for rebirth</p><h2 class="notification-title"><img class="notification-icon" src="assets/nav/DroidexNotifications.png" alt="">Droidex Notifications</h2><p>${scope} · through R: ${rebirthGoal()} · ${recommendations.length}/${slots} purchased slots recommended</p></div><button class="btn secondary" type="button" data-notification-settings>Settings</button></header>
    ${!slots?'<p>No notification slots recorded. Set your purchased level in <a href="#/nova-shop/droidex-notifications">Nova Shop</a> or Settings. Each slot costs 20 Nova Crystals, up to 10.</p>':recommendations.length?`<ol class="notification-list">${recommendations.map(rec=>{
      const d=fusionDroid(rec.name);
      return `<li>${picture(d,rec.variant)}<div><strong>${escapeAttr(rec.name)}${variantWatch?` · ${variantText(rec.variant)}`:''}</strong><small>${variantWatch?'Set this droid and variant in the game’s Droidex.':`Track this droid. Rebirth needs ${variantLabel(rec.variant)}.`} Needed at R: ${rec.needs.map(n=>n.at).join(', ')}.${rec.have?` Owned at ${variantLabel(rec.have)}; upgrading it is another option.`:''}</small></div></li>`;
    }).join('')}</ol>`:'<p>No missing spawnable droids match these settings. Your owned droids already cover them, or you can widen the rebirth range.</p>'}
    ${slots?`<p class="picker-hint">${variantWatch?'Variant Watch is owned: select the recommended variant to include it and higher variants.':'Notifications currently track the droid only. Variant Watch (100 Nova Crystals) adds variant targeting.'} These are recommendations to set in-game.${model.candidates.length>slots?` ${model.candidates.length-slots} more droids fall outside your slot limit.`:''}</p>`:''}</section>`;
}
...
function showNotificationSettings(){
  ...
  root.innerHTML=`<div class="modal-backdrop"><section class="modal planning-settings-modal" role="dialog" aria-modal="true" aria-labelledby="notificationSettingsTitle"><h2 class="notification-title" id="notificationSettingsTitle"><img class="notification-icon" src="assets/nav/DroidexNotifications.png" alt="">Droidex Notifications</h2>
    ...
    <label class="planning-check"><input type="checkbox" id="notificationUpgrades" ${settings.includeUpgrades?'checked':''}>Include droids I own below the required variant</label>
  ...
  root.querySelector('#saveNotificationSettings').onclick=()=>{
    state.notificationPreferences=normaliseNotificationPreferences({priority:root.querySelector('#notificationPriority').value,horizon:root.querySelector('#notificationHorizon').value,includeUpgrades:root.querySelector('#notificationUpgrades').checked});
...
document.addEventListener('click',event=>{
  const button=event.target.closest('[data-fusion-keep-settings],[data-reserve-fusion],[data-astromech-settings],[data-fusion-settings],[data-notification-settings]');if(!button)return;
