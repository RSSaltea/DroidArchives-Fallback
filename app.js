...
function showFusionSettings(){
  ...
  root.innerHTML=`<div class="modal-backdrop"><section class="modal planning-settings-modal" role="dialog" aria-modal="true" aria-labelledby="fusionSettingsTitle"><h2 id="fusionSettingsTitle">Fusion settings</h2>
    ...
    <p class="picker-hint">Three identical droids below Stellar still upgrade their variant. Higher rarity and Mythic rerolls choose a mixed-name batch instead. If no suitable batch exists, no fusion is recommended. Use Keep for fusion rules to save spare inputs until then.</p>
...
function normaliseFusionKeepRules(value){return Array.isArray(value)?value.filter(x=>x&&VARIANTS.includes(x.variant)&&(typeof x.name==='string'||RARITY_LADDER.includes(x.rarity))).map(x=>x.name?{name:x.name,variant:x.variant}:{rarity:x.rarity,variant:x.variant}):[];}
function keepForFusion(unit){
  const d=state.droids.find(d=>d.name===unit.name);if(!d||isIconic(d))return false;
  return normaliseFusionKeepRules(state.fusionKeepRules).some(rule=>rule.name?rule.name===unit.name&&rule.variant===unit.variant:RARITY_LADDER.indexOf(d.rarity)>=RARITY_LADDER.indexOf(rule.rarity)&&VARIANTS.indexOf(unit.variant)>=VARIANTS.indexOf(rule.variant));
}
...
function fusionBestFrom(stock,floor,made,protocolFloors){
  ...
  const permitted=options.filter(option=>{
    ...
    if(hasMythic){
      ...
      return option.kind!=='rarity'||option.variant==='STELLAR';
    }
...
function fusionOutlookHtml(){
  ...
  const rarityBlock=rarity.length?`<div class="fusion-outlook-block"><h3>Rarity steps you could take</h3>
    <p>Three droids sharing a rarity and a quality come out one rarity higher (or reroll at Mythic), at that same quality &mdash; including three of the same droid once it is Stellar and has no quality left to climb. Which droid you get is a roll. If the three you pick happen to be a recorded combination, that recipe wins instead and names its result.</p>
...
function fusionMineRows(){
  ...
  return fusionRecipes().map(recipe=>{
    ...
    const parts=need.parts.map(part=>{
      ...
      const copies=units.filter(unit=>unit.name===part.name).sort((a,b)=>Number(needed.has(a.key))-Number(needed.has(b.key))||VARIANTS.indexOf(b.variant)-VARIANTS.indexOf(a.variant));
    ...
    const at=need.ready?VARIANTS[Math.min(...parts.flatMap(part=>part.use.map(unit=>VARIANTS.indexOf(unit.variant))))]:'';
...
<section class="fusion-rules"><header><p class="eyebrow">The ladder</p><h2>How fusion works</h2></header>
      <ol>
        ...
        <li><b>Three mixed-name Mythics reroll into a Mythic</b> at the lowest variant of the inputs. Three identical Mythics still upgrade their variant, or reroll if already Stellar.</li>
...
(function () {
  ...
  const plannerMaxRebirth = shop => Math.max(12,Math.min(Number(localStorage.getItem(plannerMaxRebirthKey()))||35,Math.max(...shop.rebirthRewards.map(reward=>reward.rebirth))));
...
function archiveDroidUsefulness(name,variant){
  ...
  if(!d||!VARIANTS.includes(variant)||(!isIconic(d)&&!d.variants?.[variant])||(isIconic(d)&&variant!=='DEFAULT'))return{ready:false,message:'Droid or quality not recognised'};
...
archiveExperience=createArchiveExperience({
  ...
  variants:name=>{const d=state.droids.find(d=>d.name===name);return isIconic(d)?['DEFAULT']:VARIANTS.filter(v=>d.variants?.[v]);},
...
const DATA_VERSION='2026-09-15-rebirth-notifications';
