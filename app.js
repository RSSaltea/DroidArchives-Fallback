...
document.addEventListener('click',event=>{
  const button=event.target.closest('[data-fusion-keep-settings],[data-reserve-fusion],[data-astromech-settings]');if(!button)return;
...
function baseExport(){return{app:'Droid Archives',version:5,exportedAt:new Date().toISOString(),base:{owned:state.owned,blueprints:state.blueprints,droidex:state.droidex,novaUpgrades:state.novaUpgrades,cantinaPurchases:state.cantinaPurchases,multiplier:state.multiplier,cycle:state.cycle,rebirth:state.rebirth,superRebirthGoal:state.superRebirthGoal,astromechIconicRoles:normaliseAstromechIconicRoles(state.astromechIconicRoles),fusionKeepRules:state.fusionKeepRules||[],protocolPriority:state.protocolPriority,optimiseFreeBuild:state.optimiseFreeBuild,optimiseFreeBuildMode:state.optimiseFreeBuildMode,optimiseKeepDroidex:state.optimiseKeepDroidex,companionGoals:state.companionGoals,preferredCompanions:state.preferredCompanions,autoCompleteBuilds:state.autoCompleteBuilds,autoPurchaseSlots:state.autoPurchaseSlots,purchasedSlots:state.purchasedSlots,loungePurchased:state.loungePurchased,novaLevel:state.novaLevel,rebirthTracker:state.rebirthTracker}}}
...
const DATA_VERSION='2026-09-14-astromech-settings';
