...
(async()=>{
 ...
 await page.route('**/app.js*',r=>r.fulfill({contentType:'text/javascript',body:source+'\nwindow.fusionApplyTest={state,save,route,placements,optimiseBase,incomeForPlaced,optimisedPlacements,safeOptimiseStepPlan,validateBaseImport,baseExport,autoPurchaseEligibleSlots,stationSlotIndices};'}));
 await page.goto(base+'/#/base');await page.waitForFunction(()=>window.fusionApplyTest?.state.droids.length);
 ...
 await page.goto(base+'/#/optimise');await page.locator('#applyOptimised').click();await page.waitForSelector('#fusionResultCancel');
 ...
 assert.deepEqual(errors,[]);console.log('PASS: two mixed Galactic rolls, cancellation, exact input consumption, selected outputs and saved reload');
