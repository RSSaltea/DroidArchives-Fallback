...
(async()=>{
 ...
 await page.route('**/app.js*',r=>r.fulfill({contentType:'text/javascript',body:source+'\nwindow.fusionApplyTest={state,save,route,placements,optimiseBase,incomeForPlaced,optimisedPlacements,safeOptimiseStepPlan,validateBaseImport,baseExport,autoPurchaseEligibleSlots,stationSlotIndices,createOptimisePreview,applyOptimisedLayout,slotLogSession,forceUnreachablePlan:()=>{state.optimiseFuseFirst=false;optimisePreviewCache=null;safeOptimiseStepPlan=(baseP,projected)=>{projected.planComplete=false;projected.planIssues=["Unreachable work destination"];projected.later=[];return [{type:"note",at:"WORKER",visit:"route-1",planBlocked:true,text:"Unreachable work destination"}]};}};'}));
